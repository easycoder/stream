!   admin-main.as
!   Merged admin panel for stream.eclecity.net.
...
!   Parsers and formatters convert at input/render boundaries.
    ...
    button DocumentUrlBtn
    button DocumentUploadBtn
    div ModalStatus
    button DeleteBtn
    button CancelBtn
    button SaveBtn
...
!!! style strings, then show the login panel.
    ...
    attach DocumentUrlBtn to `document-url-btn`
    attach DocumentUploadBtn to `document-upload-btn`
...
PopulateForm:
    ...
    put property `document_url` of Row into TempStr
    set the content of DocumentUrlInput to TempStr
    if TempStr is not empty
...
OnEmail1:
    ...
    put the content of NameInput into BookingName
    gosub MakeSlug
    put `https://stream.eclecity.net/` cat TempStr into StreamUrl
